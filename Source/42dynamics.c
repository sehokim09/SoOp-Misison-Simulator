...
void SCMassProps(struct SCType *S)
{
      ...
      for(i=0;i<3;i++) {
         for(j=0;j<3;j++) S->J[i][j] = B0->J[i][j];
      }
      MxV(B0->CN,B0->pn,p);
      p2 = VoV(p,p);
      for(i=0;i<3;i++) {
      ...
      for(i=0;i<3;i++) {
         for(j=0;j<3;j++) S->J[i][j] += B0->mass*pp[i][j];
      }
      for(Ib=1;Ib<S->Nb;Ib++) {
         B = &S->B[Ib];
         MxMT(B->CN,B0->CN,CI0);
         MxV(B0->CN,B->pn,p);
         PARAXIS(B->J,CI0,B->mass,p,MOI);
         for(i=0;i<3;i++) {
            for(j=0;j<3;j++) S->J[i][j] += MOI[i][j];
         }
      }
}
/**********************************************************************/
      ...
      for(Ib=1;Ib<S->Nb;Ib++) {
         ...
         for(i=0;i<3;i++) {
            for(j=0;j<G->RotDOF;j++) {
               ...
               for(k=0;k<3;k++) {
                  D->IPAngVel[i0+i][j0+j] += Bib->J[i][k]*CGo[k][j];
               }
            }
         }
         Jb = G->Bin;
         while(Jb > 0) {
            ...
            for(i=0;i<3;i++) {
               for(j=0;j<G->RotDOF;j++) {
                  ...
                  D->IPAngVel[i0+i][j0+j] = 0.0;
                  for(k=0;k<3;k++) {
                     D->IPAngVel[i0+i][j0+j] += Bib->J[i][k]*CG[k][j];
                  }
               }
            }
            Jb = G->Bin;
         }
         MxM(Bib->J,D->BodyPathTable[Ib][0].Coi,IC);
         for(i=0;i<3;i++) {
            for(j=0;j<3;j++) {
      ...
      for(Ib=1;Ib<S->Nb;Ib++) {
         ...
         for(i=0;i<3;i++) {
            for(j=0;j<Bib->Nf;j++) {
               ...
               for(k=0;k<3;k++)
               D->IPAngVelf[i0+i][j0+j] += Bib->J[i][k]*D->PAngVelf[i0+k][j0+j];
            }
         }
         Jb = Gi->Bin;
         while (Jb > 0) {
               ...
               for(j=0;j<Bjb->Nf;j++) {
                  ...
                  for(k=0;k<3;k++) {
                     D->IPAngVelf[i0+i][j0+j] +=
                        Bib->J[i][k]*D->PAngVelf[i0+k][j0+j];
                  }
               }
            }
            Jb = Gi->Bin;
